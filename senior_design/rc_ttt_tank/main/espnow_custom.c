...
void target_tracking_task(void *pvParameter) {
    ...
    //adc1_config_channel_atten(ADC1_CHANNEL_0, ADC_ATTEN_DB_11); //IR_S_2 --Unused/Unusable
    ...
    while (1) {
        ...
        if(rotate_turret == 1){
            for (int i = 0; i < channels; i++){
                ...
                ESP_LOGI(TAG, "Original IR_S_%d Raw Data: %d, Voltage: %f", i+4, raw_data[i], v_data[i]);
            }
...
void firing_task(void *pvParameter) {
    ...
    while (1) {
        ...
        vTaskDelay(100 / portTICK_PERIOD_MS); // Release for 100 milliseconds
    }
}
